import * as THREE from 'three';
import { GLTFLoader } from 'three/addons/loaders/GLTFLoader.js';

export const planetData = {
    Mercury: {
        radius: 0.4,
        distance: 10,
        color: 0xaaaaaa,
        speed: 0.04,
        desc: "Smallest planet.",
        isTextured: true,
        dayTex: 'https://assets.science.nasa.gov/dynamicimage/assets/science/psd/solar/2023/09/p/i/a/1/PIA17386.jpg?w=4096&h=2048&fit=clip&crop=faces%2Cfocalpoint',
        rimColor: new THREE.Vector3(0.7, 0.7, 0.7)
    },
    Venus:   {
        radius: 0.8,
        distance: 15,
        color: 0xe3bb76,
        speed: 0.03,
        desc: "Earth's sister.",
        isTextured: true,
        dayTex: 'https://upload.wikimedia.org/wikipedia/commons/1/1c/Solarsystemscope_texture_8k_venus_surface.jpg',
        rimColor: new THREE.Vector3(0.9, 0.8, 0.6)
    },
    Earth:   {
        radius: 0.9,
        distance: 20,
        color: 0x2233ff,
        speed: 0.02,
        desc: "Our home.",
        isEarth: true,
        dayTex: 'https://unpkg.com/three-globe/example/img/earth-blue-marble.jpg',
        nightTex: 'https://unpkg.com/three-globe/example/img/earth-night.jpg',
        rimColor: new THREE.Vector3(0.3, 0.6, 1.0)
    },
    Mars:    {
        radius: 0.7,
        distance: 25,
        color: 0xff4422,
        speed: 0.015,
        desc: "The Red Planet.",
        isMars: true,
        dayTex: 'https://upload.wikimedia.org/wikipedia/commons/e/ea/Mars_%284997052786%29.jpg',
        rimColor: new THREE.Vector3(1.0, 0.4, 0.2)
    },
    Jupiter: {
        radius: 2.0,
        distance: 35,
        color: 0xd39c7e,
        speed: 0.01,
        desc: "Massive gas giant, largest in our system.",
        isTextured: true,
        dayTex: 'https://upload.wikimedia.org/wikipedia/commons/b/be/Solarsystemscope_texture_2k_jupiter.jpg',
        rimColor: new THREE.Vector3(0.8, 0.7, 0.6)
    },
    Saturn:  {
        radius: 1.7,
        distance: 45,
        color: 0xc5ab6e,
        speed: 0.008,
        desc: "Famous for its stunning ring system.",
        isTextured: true,
        dayTex: 'https://upload.wikimedia.org/wikipedia/commons/1/1e/Solarsystemscope_texture_8k_saturn.jpg',
        hasRings: true,
        rimColor: new THREE.Vector3(0.8, 0.7, 0.5)
    },
    Uranus:  {
        radius: 1.2,
        distance: 55,
        color: 0xbbe1e4,
        speed: 0.006,
        desc: "Ice giant, rotates on its side.",
        isTextured: true,
        dayTex: 'https://upload.wikimedia.org/wikipedia/commons/9/95/Solarsystemscope_texture_2k_uranus.jpg',
        rimColor: new THREE.Vector3(0.6, 0.8, 0.9)
    },
    Neptune: {
        radius: 1.1,
        distance: 65,
        color: 0x6081ff,
        speed: 0.005,
        desc: "Cold, windy, blue ice giant.",
        isTextured: true,
        dayTex: 'https://upload.wikimedia.org/wikipedia/commons/1/1e/Solarsystemscope_texture_2k_neptune.jpg',
        rimColor: new THREE.Vector3(0.4, 0.5, 0.9)
    },
    Pluto:   {
        radius: 0.2,
        distance: 75,
        color: 0x888888,
        speed: 0.004,
        desc: "Dwarf planet, located in the Kuiper belt.",
        isTextured: true,
        dayTex: 'https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/7117654f-5f91-4761-bde0-727b9abb0112/di7sqt5-6e5aaf23-ba34-48aa-8c2a-31eb5d9e8615.png/v1/fill/w_1264,h_632,q_70,strp/pluto_texture_map__fixed_blur___unmaped_areas__by_4stron4omi4_di7sqt5-pre.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..2ZqKkITp3qZg77C-0-ff-cWuvaADPwq82nCx2Bu39mA',
        rimColor: new THREE.Vector3(0.6, 0.6, 0.6)
    }
};

const loader = new GLTFLoader();

export class Planet {
    constructor(scene, name, data) {
        this.name = name;
        this.desc = data.desc;
        this.distance = data.distance;
        this.speed = data.speed;
        this.radius = data.radius;
        this.angle = Math.random() * Math.PI * 2;

        this.orbitGroup = new THREE.Group();
        scene.add(this.orbitGroup);

        this.mesh = new THREE.Group(); // Container for the planet
        this.mesh.userData.planet = this;
        this.orbitGroup.add(this.mesh);
        this.mesh.position.x = data.distance;

        if (data.model) {
            this.loadModel(data);
        } else if (data.isEarth || data.isMars || data.isTextured) {
            this.planetMesh = this.createPlanetWithShader(data);
            this.mesh.add(this.planetMesh);
        } else {
            const geo = new THREE.SphereGeometry(this.radius, 32, 32);
            const mat = new THREE.MeshStandardMaterial({ color: data.color });
            this.planetMesh = new THREE.Mesh(geo, mat);
            this.mesh.add(this.planetMesh);
        }

        // Add Rings if needed
        if (data.hasRings) {
            this.createRings();
        }
    }

    createRings() {
        loader.load('models/asteroids.glb', (gltf) => {
            let asteroidMesh = null;
            gltf.scene.traverse(child => {
                if (child.isMesh && !asteroidMesh) {
                    asteroidMesh = child;
                }
            });

            if (!asteroidMesh) return;

            const particleCount = 8000;
            const instancedMesh = new THREE.InstancedMesh(asteroidMesh.geometry, asteroidMesh.material, particleCount);

            const dummy = new THREE.Object3D();
            for (let i = 0; i < particleCount; i++) {
                const angle = Math.random() * Math.PI * 2;
                const distance = THREE.MathUtils.randFloat(this.radius * 1.5, this.radius * 2.5);

                const x = Math.cos(angle) * distance;
                const y = (Math.random() - 0.5) * 0.15;
                const z = Math.sin(angle) * distance;

                dummy.position.set(x, y, z);
                dummy.rotation.set(Math.random() * Math.PI, Math.random() * Math.PI, Math.random() * Math.PI);

                // Scale it to be extremely tiny to match previous rock look
                const scale = THREE.MathUtils.randFloat(0.001, 0.005);
                dummy.scale.set(scale, scale, scale);

                dummy.updateMatrix();
                instancedMesh.setMatrixAt(i, dummy.matrix);
            }

            instancedMesh.instanceMatrix.needsUpdate = true;
            this.mesh.add(instancedMesh);
        }, undefined, (error) => {
            console.error("Error loading asteroids model:", error);
            // Fallback to original rocks if loading fails
            this.createFallbackRings();
        });
    }

    createFallbackRings() {
        const particleCount = 5000;
        const group = new THREE.Group();
        const rockGeometry = new THREE.TetrahedronGeometry(0.03, 0);
        const rockMaterial = new THREE.MeshBasicMaterial({ color: 0x998877 });

        for (let i = 0; i < particleCount; i++) {
            const rock = new THREE.Mesh(rockGeometry, rockMaterial);
            const angle = Math.random() * Math.PI * 2;
            const distance = THREE.MathUtils.randFloat(this.radius * 1.5, this.radius * 2.5);
            const x = Math.cos(angle) * distance;
            const y = (Math.random() - 0.5) * 0.15;
            const z = Math.sin(angle) * distance;
            rock.position.set(x, y, z);
            rock.rotation.set(Math.random() * Math.PI, Math.random() * Math.PI, Math.random() * Math.PI);
            group.add(rock);
        }
        this.mesh.add(group);
    }

    loadModel(data) {
        loader.load(data.model, (gltf) => {
            const model = gltf.scene;

            // Center and scale
            const box = new THREE.Box3().setFromObject(model);
            const size = box.getSize(new THREE.Vector3());
            const center = box.getCenter(new THREE.Vector3());
            model.position.sub(center);

            const maxDim = Math.max(size.x, size.y, size.z);
            const scale = (this.radius * 2) / maxDim;
            model.scale.setScalar(scale);

            // Apply shader if textures provided
            if (data.dayTex) {
                const texLoader = new THREE.TextureLoader();
                const dayTex = texLoader.load(data.dayTex);
                const nightTex = data.nightTex ? texLoader.load(data.nightTex) : dayTex;

                const material = this.getShaderMaterial(dayTex, nightTex, data.rimColor, !!data.nightTex);

                model.traverse(child => {
                    if (child.isMesh) {
                        child.material = material;
                    }
                });
            }

            this.mesh.add(model);
            this.planetMesh = model;
        });
    }

    createPlanetWithShader(data) {
        const texLoader = new THREE.TextureLoader();
        const dayTex = texLoader.load(data.dayTex);
        const nightTex = texLoader.load(data.nightTex || data.dayTex);

        const material = this.getShaderMaterial(dayTex, nightTex, data.rimColor, !!data.nightTex);
        return new THREE.Mesh(new THREE.SphereGeometry(this.radius, 64, 64), material);
    }

    getShaderMaterial(dayTex, nightTex, rimColor, hasNightTex) {
        return new THREE.ShaderMaterial({
            uniforms: {
                dayTexture: { value: dayTex },
                nightTexture: { value: nightTex },
                sunDirection: { value: new THREE.Vector3(1, 0, 0) }, // Default non-zero
                rimColor: { value: rimColor || new THREE.Vector3(1, 1, 1) },
                hasNightTexture: { value: hasNightTex ? 1.0 : 0.0 }
            },
            vertexShader: `
                varying vec2 vUv;
                varying vec3 vNormal;
                varying vec3 vViewDir;
                void main() {
                    vUv = uv;
                    vec4 worldPos = modelMatrix * vec4(position, 1.0);
                    vNormal = normalize(vec3(modelMatrix * vec4(normal, 0.0)));
                    vViewDir = normalize(cameraPosition - worldPos.xyz);
                    gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
                }
            `,
            fragmentShader: `
                uniform sampler2D dayTexture;
                uniform sampler2D nightTexture;
                uniform vec3 sunDirection;
                uniform vec3 rimColor;
                uniform float hasNightTexture;
                varying vec2 vUv;
                varying vec3 vNormal;
                varying vec3 vViewDir;
                void main() {
                    vec3 sDir = normalize(sunDirection);
                    float intensity = dot(normalize(vNormal), sDir);
                    float dayWeight = smoothstep(-0.15, 0.15, intensity);

                    vec3 dayColor = texture2D(dayTexture, vUv).rgb;
                    vec3 nightColor = texture2D(nightTexture, vUv).rgb;

                    if (hasNightTexture < 0.5) {
                        nightColor = dayColor * 0.15; // Slightly brighter night for visibility
                    } else {
                        nightColor *= 2.5;
                    }

                    vec3 color = mix(nightColor, dayColor, dayWeight);

                    // Atmospheric rim glow
                    float fresnel = pow(1.0 - dot(vNormal, vViewDir), 3.0);
                    color += rimColor * fresnel * 0.6;

                    gl_FragColor = vec4(color, 1.0);
                }
            `
        });
    }

    update(stopOrbit = false) {
        if (!stopOrbit) {
            this.angle += this.speed;
            this.orbitGroup.rotation.y = this.angle;
        }
        this.mesh.rotation.y += 0.005;

        // Find the actual mesh/model that has the material
        let target = null;
        if (this.planetMesh) {
            if (this.planetMesh.isMesh) {
                target = this.planetMesh;
            } else {
                this.planetMesh.traverse(child => {
                    if (child.isMesh && child.material && child.material.uniforms) {
                        target = child;
                    }
                });
            }
        }

        if (target && target.material && target.material.uniforms && target.material.uniforms.sunDirection) {
            const worldPos = new THREE.Vector3();
            target.getWorldPosition(worldPos);
            // Sun is at (0,0,0)
            target.material.uniforms.sunDirection.value.copy(worldPos).negate().normalize();
        }
    }
}
